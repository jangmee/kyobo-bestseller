...
def parse_kyobo(html: str, now: str) -> list:
...
def parse_aladin(html: str, now: str) -> list:
    soup = BeautifulSoup(html, 'html.parser')
    books = []
    for div in soup.find_all('div', class_='ss_book_list'):
        ...
        books.append({"수집시각": now, "순위": str(len(books)+1), "제목": title,
                      "저자": author, "출판사": publisher, "링크": link,
                      "이전순위": "", "순위변동": ""})
        if len(books) >= 100:
            break
...
def parse_yes24(html: str, now: str) -> list:
    soup = BeautifulSoup(html, 'html.parser')
    books = []
    seen = set()
...
def scrape_all(now: str) -> dict:
    ...
    print("🌐 알라딘 수집 중...")
    try:
        html = fetch_html("https://www.aladin.co.kr/shop/common/wbest.aspx?BranchType=1&BestType=NowBest")
        results["aladin"] = parse_aladin(html, now)
...
def save_csv(store: str, books: list):
...
def save_history(all_books: dict, now: str):
    path = Path(HISTORY_FILE)
    history = []
    if path.exists():
        with open(path, encoding="utf-8") as f:
